...
export class AdmTreeNodeComponent {
  ...
  @Output() addMember = new EventEmitter<{ bossId: number }>();
  ...
  onAddMember(): void {
    this.addMember.emit({ bossId: this.node.userId });
  }
  ...
  onChildAdd(event: { bossId: number }): void {
    this.addMember.emit(event);
  }

  onChildEdit(event: AdmHierarchyTeam): void {
    this.editMember.emit(event);
  }

  onChildDelete(event: AdmHierarchyTeam): void {
    this.deleteMember.emit(event);
  }
}
