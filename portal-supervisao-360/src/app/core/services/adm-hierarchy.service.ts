...
export class AdmHierarchyService {
  private base = `${environment.apiUrl}/AdmHierarchy`;

  constructor(private http: HttpClient) {}

  select(filters: Record<string, any> = {}): Observable<AdmHierarchy[]> {
    let params = new HttpParams();
    Object.entries(filters).forEach(([k, v]) => {
      if (v != null && v !== '') params = params.set(k, v);
    });
    return this.http.get<AdmHierarchy[]>(`${this.base}/Select`, { params });
  }
