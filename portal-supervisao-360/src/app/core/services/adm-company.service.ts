...
export class AdmCompanyService {
  private base = `${environment.apiUrl}/AdmCompany`;

  constructor(private http: HttpClient) {}

  select(filters: Record<string, any> = {}): Observable<AdmCompany[]> {
    let params = new HttpParams();
    Object.entries(filters).forEach(([k, v]) => {
      if (v != null && v !== '') params = params.set(k, v);
    });
    return this.http.get<AdmCompany[]>(`${this.base}/Select`, { params });
  }
