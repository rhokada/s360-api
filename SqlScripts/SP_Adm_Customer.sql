-- =============================================================================
-- SP_Adm_Customer — CRUD unificado de cliente controlado por @TypeRequest
-- =============================================================================
IF OBJECT_ID('SP_Adm_Customer', 'P') IS NOT NULL DROP PROCEDURE SP_Adm_Customer;
GO
CREATE PROCEDURE SP_Adm_Customer
...
BEGIN
    ...
    BEGIN
        ...
        WHERE
            (@CompanyId     IS NULL OR cu.CompanyId     = @CompanyId)
            AND (@Name          IS NULL OR cu.Name          LIKE '%' + @Name + '%')
            AND (@CustomerCode  IS NULL OR cu.CustomerCode  = @CustomerCode)
            AND (@CNPJ          IS NULL OR cu.CNPJ          = @CNPJ)
            AND (@City          IS NULL OR cu.City          = @City)
            AND (@State         IS NULL OR cu.State         = @State)
...
END
GO
GRANT EXECUTE ON SP_Adm_Customer TO S360sys;
GO
