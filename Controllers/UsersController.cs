...
namespace WebApi.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        [AllowAnonymous]
        [HttpPost("authenticate-adm")]
        public IActionResult AuthenticateAdm([FromBody] AuthenticateModel model)
        {
            /*if (!_reCaptchaService.ValidaReCaptcha(model.recaptcha))
               return BadRequest(new { message = "ReCaptcha inválido" });*/

            var user = _userService.AuthenticateAdm(model.Username, model.Password, model.AppId);
